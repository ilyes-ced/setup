...
fn read_scheme_proprety() -> Result<Value, ()> {
    let binding = read_to_string("propreties/zenburn.json").unwrap();
    let colors = binding.as_str();
    let json: Value = from_str(colors).expect("JSON was not well-formatted");
    Ok(json)
}
...
fn create_json(path: PathBuf) -> Result<(), Box<dyn Error>> {
    ...
    let mut file = File::create(["themes/json/", path.file_name().unwrap().to_str().unwrap()].iter().collect::<PathBuf>())?;
...
fn create_propreties(path: PathBuf) -> Result<(), Box<dyn Error>> {
    ...
    let pp = String::from_utf8_lossy(&[path.file_stem().unwrap().to_str().unwrap().as_bytes(), b".json"].concat()).to_string();
